...
本文档基于实际代码分析，详细描述PUMC智能排课系统的排课算法实现。系统采用三阶段排课策略，实现了完整的约束检查和方案评估机制。

> **重要说明：实际生效的求解器是自定义回溯算法。**
>
> 真实执行链路是：
> `generate_schedules()` → `_schedule_timed_courses()` →
> `_constraint_satisfaction_scheduling()` → `_backtrack_scheduling()`。
>
> 历史上 `core/scheduling/engine.py` 里还有一套基于 OR-Tools CP-SAT 的实现
> （`_build_model()` / `_add_*_constraints()` / `_set_objective()` /
> `_solve_model()` / `SolutionCollector`），但它**没有任何调用方**，
> 且实现并不完整（时间约束主循环体是 `pass`，占用表永远为空；
> 校区约束与每日上限也是 `pass`；学分约束引用了不存在的
> `config.enforce_credit_requirements`）。这套死代码与回溯路径重复定义了
> 同一批约束语义，已于本轮清理中删除，`ortools` 依赖同时移除。
>
> 若将来确定改用 CP-SAT，应在约束语义和特征测试稳定后重写，
> 而不是继续维护一份不可达的半成品。
...
# 默认仅输出 WARNING 及以上；PUMC_LOG_LEVEL=DEBUG 可查看回溯搜索过程
```
